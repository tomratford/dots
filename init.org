...
#+end_src
** Tempel
This is an alternative snippet library. I find [[yasnippet][yasnippet]] to be a bit restrictive in some respects and this feels like a fun alternative.
#+begin_src elisp :var initFile=(buffer-file-name)
  (use-package tempel
    ;; :bind
    ;; (("TAB" . tempel-expand)
    ;;  ([tab] . tempel-expand))
    ...
    :custom
    (tempel-path (concat (file-name-directory initFile) "tempel"))
...
#+end_src
** Flycheck
Again, not a programming language but LSP uses it so
#+begin_src elisp
